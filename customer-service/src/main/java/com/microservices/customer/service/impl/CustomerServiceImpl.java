...
public class CustomerServiceImpl implements CustomerService {
	...
	public List<Product> getItems(long userId) {
		...
		return wishLists.stream().map(wl -> productDAO.findById(wl.getProductId()).orElse(null))
	...
	public void removeWishlistItem(long userId, long productId) {
		WishList wishList = dao.findByCustomerUserId(userId).stream().filter(wl -> wl.getProductId() == productId)
				.findFirst().orElseThrow(() -> new RuntimeException("Wishlist item not found"));
